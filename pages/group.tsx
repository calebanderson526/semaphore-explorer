                      ...
                      <tbody>
                        {group.members?.map((member, index) => (
                          <tr key={member}>
                            ...
                            <td className='text-white'>{member.slice(0, 6)} ... {member.slice(71, 77)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </Table>
                      ...
                      <tbody>
                        {group.verifiedProofs?.map((proof) => (
                          <tr key={proof.nullifierHash}>
                            <td className='text-white'>{proof.signal.slice(0, 6)} ... {proof.signal.slice(71, 77)}</td>
                            <td className='text-white'>{proof.merkleTreeRoot.slice(0, 6)} ... {proof.signal.slice(71, 77)} </td>
                            <td className='text-white'>{proof.externalNullifier}</td>
                            <td className='text-white'>{proof.nullifierHash.slice(0, 6)} ... {proof.nullifierHash.slice(71, 77)}</td>
                            <td className='text-white'>{unixTimeToString(Number(proof.timestamp))}</td>
                          </tr>
                        ))}
                      </tbody>
                    </Table> : ''
